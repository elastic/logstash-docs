[[plugins-codecs-gzip_lines]]
=== gzip_lines


NOTE: This is a community-maintained plugin! It does not ship with Logstash by default, but it is easy to install by running `bin/plugin install logstash-codec-gzip_lines`.
...
[[plugins-codecs-gzip_lines-charset]]
===== `charset` 
