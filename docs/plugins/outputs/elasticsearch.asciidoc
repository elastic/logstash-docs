...
This output speaks the HTTP protocol, the preferred protocol for
...
(see {logstash-ref}/event-dependent-configuration.html#metadata) to set the
destination index for each event. The `[@metadata]` fields will not be sent to
Elasticsearch.
...
Elasticsearch {ref}/security-api-create-api-key.html[Create API key
API].
...
prevent Logstash from sending bulk requests to the master nodes. 
This parameter should reference only data or client nodes in Elasticsearch.
