...
Set the address of a forward HTTP proxy. Can be either a string, such as `http://localhost:123` or a hash in the form
`{host: 'proxy.org' port: 80 scheme: 'http'}`. Note, this is NOT a SOCKS proxy, but a plain HTTP proxy
