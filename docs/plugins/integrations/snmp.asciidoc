...
This integrated plugin package provides better alignment in behaviour when it comes to SNMP processing, better resource management, easier package maintenance, and a smaller installation footprint. 
...
Given the underlying code refactor to better align snmp and snmptrap plugin behaviour you might need to address some changes depending on the use-case and how the ingested data is being handled through the pipeline.
