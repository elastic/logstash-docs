...
Current, pre-4.0.0 versions of `input-snmp` and `input-snmptrap` plugins are bundled with {ls} by default, and will not be upgraded to the snmp integration without your deliberate action. 

Be aware of behavioral and mapping differences between current stand-alone plugins and the new versions included in the `integration-snmp`. This topic can help. 
****
