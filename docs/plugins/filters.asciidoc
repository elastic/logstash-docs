...
| <<plugins-filters-jdbc_static>> | Enriches events with pre-loaded data from a fast in-memory SQL database  | https://github.com/logstash-plugins/logstash-filter-jdbc_static[logstash-filter-jdbc_static]
