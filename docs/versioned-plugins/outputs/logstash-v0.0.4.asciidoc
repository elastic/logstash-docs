...
:changelog_url: https://github.com/logstash-plugins/logstash-integration-logstash/blob/v0.0.4/CHANGELOG.md
...
This output plugin needs to be configured to connect to a {logstash-ref}/plugins-inputs-logstash.html[logstash input plugin] by specifying its <<{version}-plugins-{type}s-{plugin}-hosts>>.
...
If the downstream {logstash-ref}/plugins-inputs-logstash.html[logstash input plugin] is configured to require `username` and `password`,
...
| <<{version}-plugins-{type}s-{plugin}-hosts>> |{logstash-ref}/configuration-file-structure.html#string[string] |Yes
...
** `full`: verifies that a certificate provided by the client has an identity claim matching <<{version}-plugins-{type}s-{plugin}-hosts>>, is signed by a trusted authority (CA), is within its valid date range, and that the client has possession of the associated key.
