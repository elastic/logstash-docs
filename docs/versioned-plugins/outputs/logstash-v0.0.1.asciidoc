...
If the downstream {logstash-ref}/plugins-inputs-logstash.html[logstash input plugin] is configured to require <<{version}-plugins-{type}s-{plugin}-username>> and <<{version}-plugins-{type}s-{plugin}-password>>,
