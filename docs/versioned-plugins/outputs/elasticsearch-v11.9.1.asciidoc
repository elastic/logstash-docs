...
- A sprintf style string to change the action based on the content of the event. The value `%{[foo]}`
  would use the foo field for the action. If resolved action is not in [`index`, `delete`, `create`, `update`], the event will not be sent to {es}, and will instead either be sent to the pipeline's {logstash-ref}/dead-letter-queues.html[dead letter queue] if it is enabled, or will be logged and dropped.
