...
| <<{version}-plugins-{type}s-{plugin}-key_pad>> |{logstash-ref}/configuration-file-structure.html#string[string]|No
...
===== `base64`
  ...
  ** When <<{version}-plugins-{type}s-{plugin}-mode,`mode => encrypt`>>, the source ciphertext will be `base64`-decoded before it is deciphered.
  ** When <<{version}-plugins-{type}s-{plugin}-mode,`mode => decrypt`>>, the result ciphertext will be `base64`-encoded before it is stored.
...
<<{version}-plugins-{type}s-{plugin}-algorithm>>'s block size requirements.
...
The size of the IV is generally dependent on which <<{version}-plugins-{type}s-{plugin}-algorithm>>
...
The character used to pad the key to the required <<{version}-plugins-{type}s-{plugin}-key_size>>.
...
The cipher's required key size, which depends on which <<{version}-plugins-{type}s-{plugin}-algorithm>>
you are using. If a <<{version}-plugins-{type}s-{plugin}-key>> is specified with a shorter value,
it will be padded with <<{version}-plugins-{type}s-{plugin}-key_pad>>.
...
* When <<{version}-plugins-{type}s-{plugin}-mode,`mode => encrypt`>>, the `source` should be a field containing plaintext
* When <<{version}-plugins-{type}s-{plugin}-mode,`mode => decrypt`>>, the `source` should be a field containing IV + ciphertext
...
* When <<{version}-plugins-{type}s-{plugin}-mode,`mode => encrypt`>>, the IV + ciphertext result will be stored in the `target` field
* When <<{version}-plugins-{type}s-{plugin}-mode,`mode => decrypt`>>, the plaintext result will be stored in the `target` field
