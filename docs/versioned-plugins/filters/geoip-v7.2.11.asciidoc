...
:changelog_url: https://github.com/logstash-plugins/logstash-filter-geoip/blob/v7.2.11/CHANGELOG.md
...
[id="{version}-plugins-{type}s-{plugin}-database_license"]
==== Database License
...
[id="{version}-plugins-{type}s-{plugin}-database_auto"]
...
[id="{version}-plugins-{type}s-{plugin}-manage_update"]
...
When this plugin is run with <<{version}-plugins-{type}s-{plugin}-ecs_compatibility>> disabled, the MaxMind DB's fields are added directly to the <<{version}-plugins-{type}s-{plugin}-target>>.
...
When you are using a default <<{version}-plugins-{type}s-{plugin}-target>>, the templates provided by the {logstash-ref}/plugins-outputs-elasticsearch.html[elasticsearch output] map the field to an {ref}/geo-point.html[Elasticsearch Geo_point datatype].
...
| <<{version}-plugins-{type}s-{plugin}-database>> |a valid filesystem path|No
| <<{version}-plugins-{type}s-{plugin}-default_database_type>> |`City` or `ASN`|No
...
See <<{version}-plugins-{type}s-{plugin}-database_license,Database License>> for more information.

[id="{version}-plugins-{type}s-{plugin}-default_database_type"]
...
For a complete list of available fields and how they map to an event's structure, see <<{version}-plugins-{type}s-{plugin}-field-mapping,field mapping>>.
...
** `v1`, `v8`: use fields that are compatible with Elastic Common Schema. Example: `[client][geo][country_name]`. See <<{version}-plugins-{type}s-{plugin}-field-mapping,field mapping>> for more info.
