...
:changelog_url: https://github.com/logstash-plugins/logstash-filter-kv/blob/v4.6.0/CHANGELOG.md
...
| <<{version}-plugins-{type}s-{plugin}-allow_empty_values>> |{logstash-ref}/configuration-file-structure.html#boolean[boolean]|No
...
[id="{version}-plugins-{type}s-{plugin}-allow_empty_values"]
===== `allow_empty_values`
...
NOTE: Parsing empty values typically requires <<{version}-plugins-{type}s-{plugin}-whitespace,`whitespace => strict`>>.
