...
| <<{version}-plugins-{type}s-{plugin}-cutoff_second>> |{logstash-ref}/configuration-file-structure.html#number[number]|No
...
[id="{version}-plugins-{type}s-{plugin}-cutoff_second"]
===== `cutoff_second`
...
Objects modified within this cutoff period will be processed the next time the file list is checked (relates to <<{version}-plugins-{type}s-{plugin}-interval>>).
Value is in seconds.
