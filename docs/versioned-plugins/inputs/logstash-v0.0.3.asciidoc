...
Doing so requires connecting {logstash-ref}/plugins-outputs-logstash.html[logstash output plugin] clients to provide matching <<{version}-plugins-{type}s-{plugin}-username>> and <<{version}-plugins-{type}s-{plugin}-password>>.
