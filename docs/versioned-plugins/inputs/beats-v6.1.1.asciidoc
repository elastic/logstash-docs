...
:changelog_url: https://github.com/logstash-plugins/logstash-input-beats/blob/v6.1.1/CHANGELOG.md
...
of the client's connection. When <<{version}-plugins-{type}s-{plugin}-ecs_compatibility,ECS compatibility mode>> is
enabled these are now moved in ECS compatible namespace. Here's how <<{version}-plugins-{type}s-{plugin}-ecs_compatibility,ECS compatibility mode>> affects output.
