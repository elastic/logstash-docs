...
:changelog_url: https://github.com/logstash-plugins/logstash-input-tcp/blob/v6.2.0/CHANGELOG.md
...
the sender's name, ip, and outbound port.       l|[@metadata][input][tcp][source,shell][name] l|[host]
l|[@metadata][input][tcp][source,shell][ip]   l|[@metadata][ip_address]
l|[@metadata][input][tcp][source,shell][port] l|[port]
