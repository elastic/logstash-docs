...
--[no-]log-in-json
  Specify that Logstash should write its own logs in JSON form - one event per line. If false, 
  Logstash will log using Ruby's Object#inspect (not easy to machine-parse)
