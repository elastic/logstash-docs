...
You can test for the presence or absence of a field:

[source,js]
----------------------------------
output {
  # Alerts pagerduty if the `halted` field is present
  if [halted]  {
    pagerduty {
    ...
    }
...
----------------------------------
output {
  # Drops messages without timestamps
  if ![timestamp]  {
    drop {}
  }
}
...
[float]
[[metadata]]
==== The @metadata field added[1.5]

The contents of the `@metadata` field are not part of any of your events at output time. You can use
the `@metadata` field for conditionals or to extend and build event fields with field reference and 
`sprintf` formatting.
